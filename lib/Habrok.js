...
Habrok.prototype.request = function request(raw, opts) {
  ...
  const req = {
    // [ZL] Set a default timeout to 30s, this can be overridden by the req.timeout
    pool: this.POOL,
    timeout: 30 * 1000,
    ...raw
  };
  ...
  return new Promise((resolve, reject) => requestjs(req, (err, res, body) => {
    ...
    if (errorCode === ECONNRESET || statusCode > 299) {
      if ((!err && self.RETRY_CODES.indexOf(statusCode) === -1) || attempt >= self.RETRIES) {
        debug('fail', req);
        debugRequest({ attempts: attempt });

        return reject(err || boom.boomify(new Error(), { statusCode, data: body }));
      }

      return setTimeout(
        () => {
          debug('retry', raw);
          const returnedError = err || boom.boomify(new Error(), { statusCode, data: body });
          onRetry(statusCode, returnedError);

          resolve(self.request(raw, { ...options, attempt: attempt + 1 }));
        },
        integer(
